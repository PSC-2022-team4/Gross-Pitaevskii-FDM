...
GridPoint::GridPoint(float x_, float y_, std::complex<float> wave_function_) : x(x_), y(y_), wave_function(wave_function_){}
GridPoint::~GridPoint(){};
...
void BaseSpatialGrid::normalize(){
    ...
    for (auto i = 0; i < this->num_grid_1; ++i){
        for (auto j = 0; j < this->num_grid_2;++j){
            ...
            sum += (wave_func.real() * wave_func.real() + wave_func.imag()*wave_func.imag())*infinitesimal_distance_1*infinitesimal_distance_2;
        }
    }
    sum = std::sqrt(sum);
    for (auto i = 0; i < this->num_grid_1; ++i)
